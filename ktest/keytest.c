...
int keyb_svgalib_init(int keyb_id, _Bool disable_special)
{
	struct keyb_pair* i;
	unsigned j;
...
int keyb_svgalib_enable(_Bool graphics)
{
	log_std(("keyb:svgalib: keyb_svgalib_enable(graphics:%d)\n", (int)graphics));

	if (keyboard_init() != 0) {
		fprintf(stderr, "Error enabling the svgalib keyboard driver. Function keyboard_init() failed.\n");
		return -1;
	}
...
_Bool keyb_svgalib_has(unsigned keyboard, unsigned code)
{
...
int keyb_svgalib_load(void* context)
{
	return 0;
}

void keyb_svgalib_reg(void* context)
{
}

int main(int argc, char **argv)
{
	keyb_svgalib_init(1, false);
	keyb_svgalib_enable(false);
	unsigned char code_map[KEYB_MAX];
	int i;

	while(1) {
		keyb_svgalib_poll();
		keyb_svgalib_all_get(1, code_map);
		if (code_map[KEYB_ESC])
			break;
		for (i=0; i<KEYB_MAX; i++) {
			if (code_map[i])
				printf("PRESSED: %s\n", key_name(i));
